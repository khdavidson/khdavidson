...
stellako.bio$time_trap_closed <- factor(stellako.bio$time_trap_closed, levels=c("21:00", "22:00", "23:00", "00:00", "01:00", "02:00", "03:00"), ordered=T)

stellako.lf <- stellako.lf %>%
  filter(!is.na(date_closed)) %>%
  mutate(date_opened = as.Date(stellako.lf$date_opened, origin="1899-12-30")) %>% 
  mutate(date_closed = as.Date(stellako.lf$date_closed, origin="1899-12-30")) %>% 
  print()

stellako.enviro <- stellako.enviro %>%
...
nadleh.bio$time_trap_closed <- factor(nadleh.bio$time_trap_closed, levels=c("21:00", "22:00", "23:00", "00:00", "01:00", "02:00", "03:00"), ordered=T)

nadleh.lf <- nadleh.lf %>%
  mutate(date_opened = as.Date(nadleh.lf$date_opened, origin="1899-12-30")) %>% 
  mutate(date_closed = as.Date(nadleh.lf$date_closed, origin="1899-12-30")) %>% 
  print()

nadleh.enviro <- nadleh.enviro %>%
  rename(date_opened=date.opened,
         date_closed=date.closed,
         date_true=true.date)
...
nightly_plot_fx <- function(catch_data){
  catch_data %>% 
    filter(!is.na(time_trap_closed)) %>%
    group_by(date_closed) %>%
    summarize(night_count = sum(n_unmarked_sampled+n_unmarked_spilled+n_unmarked_dead))
}



hourly_fx <- function(data) {
  data %>% 
    filter(!is.na(time_trap_closed)) %>%
...
enviro_plot <- function(site_data, span_val){
  ggplot() +
    geom_smooth(data=site_data%>%filter(!is.na(water_temp_C)), aes(x=date_opened, y=water_temp_C), size=1, colour="orange", se=F, span=span_val) +
    geom_smooth(data=site_data, aes(x=date_opened, y=water_gauge_m), size=1, colour="blue", se=F, span=span_val) +
    geom_point(data=site_data%>%filter(!is.na(water_temp_C)), aes(x=date_opened, y=water_temp_C), size=3.5, shape=21, fill="#FFBB19", colour="#FF8100") +
    geom_point(data=site_data, aes(x=date_opened, y=water_gauge_m), size=3.5, shape=21, fill="#539bff", colour="#531eff") +
...
nightly_plot <- function(catch_data, max_catch, interval){
  ggplot(data=catch_data %>% 
                ...
                group_by(date_closed) %>%
                summarize(night_count = sum(n_unmarked_sampled+n_unmarked_spilled+n_unmarked_dead)), 
      aes(x=date_closed, y=night_count)) +
    ...
    scale_x_date(date_breaks="1 day", date_labels="%b %d") +   #limits=c(as.Date("2021-04-12"), as.Date("2021-04-30")), 
...
Water temperatures are still cold in the Stellako River, on average `r round(mean(stellako.enviro$water_temp_C,na.rm=T),1)`&pm;`r round(sd(stellako.enviro$water_temp_C,na.rm=T),1)` &deg;C (Figure 1), as are air temperatures (`r round(mean(stellako.enviro$air_temp_C,na.rm=T),1)`&pm;`r round(sd(stellako.enviro$air_temp_C,na.rm=T),1)` &deg;C). Water levels have increased considerably since RST install (average of `r round(mean(stellako.enviro$water_gauge_m, na.rm=T),1)`&pm;`r round(sd(stellako.enviro$water_gauge_m,na.rm=T),2)` m), resulting in flooding of most of the lower river channels (Figure 1).

```{r, message=F, echo=F, warning=F}
enviro_plot(stellako.enviro, 0.33)
```

*Figure 1. Water temperature (degrees Celcius) and level (meters) in the Stellako River from `r format(min(stellako.enviro$date_opened), "%b %d")` to `r format(max(stellako.enviro$date_opened), "%b %d")`, 2021. Water level data obtained from real-time Environment Canada water data online.*

\newpage

**Abundance and migration timing**

Catches are still low at Stellako, with no new smolts encountered since last update (Figure 2a). Smolt migration is anticipated to be closer to early-mid May. Catches are currently too low to discern reliable hourly abundance patterns (Figure 2b), but historical data suggest peak migration should occur between 23:00-02:00. 

```{r, warning=F, message=F, echo=F}
ggarrange(nightly_plot(stellako.catch, 10, 1), hourly_plot(stellako.catch), nrow=2)
```
...
Smolts encountered at the Stellako River site are large, on average `r round(mean(stellako.bio$length_mm,na.rm=T),1)`&pm;`r round(sd(stellako.bio$length_mm, na.rm=T),1)` mm and `r round(mean(stellako.bio$weight_g,na.rm=T),1)`&pm;`r round(sd(stellako.bio$weight_g,na.rm=T),1)` g. 
...
Water temperatures have increased in the Nadleh River, but are still cold, on average `r round(mean(nadleh.enviro$water_temp_C,na.rm=T),1)`&pm;`r round(sd(nadleh.enviro$water_temp_C,na.rm=T),1)` &deg;C, as are air temperatures (`r round(mean(nadleh.enviro$air_temp_C,na.rm=T),1)`&pm;`r round(sd(nadleh.enviro$air_temp_C,na.rm=T),1)` &deg;C). Water levels have also increased (average of `r round(mean(nadleh.enviro$water_gauge_m, na.rm=T),1)`&pm;`r round(sd(nadleh.enviro$water_gauge_m,na.rm=T),2)` m, Figure 3). Ice has now completely left Fraser Lake and no longer impacts RST operations. 

```{r, message=F, echo=F, warning=F}
enviro_plot(nadleh.enviro, 0.30)
```

*Figure 3. Water temperature (degrees Celcius) and level (meters) in the Nadleh River from `r format(min(nadleh.enviro$date_opened), "%b %d")` to `r format(max(nadleh.enviro$date_opened), "%b %d")`, 2021. Water level data obtained from real-time Environment Canada water data online.*

\newpage

**Abundance and migration timing**

Catches have increased dramatically at Nadleh as the anticipated peak of migration approaches, estimated to be mid-late Apr but potentially as late as mid-May (Figure 4a). Peak nightly migration appears to be between 10pm and 1am, as it has been historically (Figure 4b). 

```{r, warning=F, message=F, echo=F}
ggarrange(nightly_plot(nadleh.catch, 4000, 500), hourly_plot(nadleh.catch), nrow=2)
```
...
Smolts encountered at the Nadleh River are slightly larger than those caught at Stellako, on average `r round(mean(nadleh.bio$length_mm,na.rm=T),1)`&pm;`r round(sd(nadleh.bio$length_mm, na.rm=T),1)` mm and `r round(mean(nadleh.bio$weight_g,na.rm=T),1)`&pm;`r round(sd(nadleh.bio$weight_g,na.rm=T),1)` g. 
...
Next update will be: `r Sys.Date()+3`
...
saveWorkbook(northern_wb, "Northern sockeye smolt programs 2021 - In-season data update #2.xlsx", overwrite = T)
```
