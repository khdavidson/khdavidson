...
enviro_plot <- function(site_data, span_val){
  ggplot() +
    ...
    geom_point(data=enviro_fx(site_data), aes(x=date_closed, y=water_temp_C), size=3.5, shape=21, fill="#FFBB19", colour="#FF8100") +
    geom_point(data=enviro_fx(site_data), aes(x=date_closed, y=water_gauge_m), size=3.5, shape=21, fill="#539bff", colour="#531eff") +
...
Water temperatures are still cold in the Stellako River, on average `r round(mean(stellako.enviro$water_temp_C,na.rm=T),1)`&pm;`r round(sd(stellako.enviro$water_temp_C,na.rm=T),1)` &deg;C (Figure 1), as are air temperatures (`r round(mean(stellako.enviro$air_temp_C,na.rm=T),1)`&pm;`r round(sd(stellako.enviro$air_temp_C,na.rm=T),1)` &deg;C). Water levels have increased considerably since RST install (average of `r round(mean(stellako.enviro$water_gauge_m, na.rm=T),1)`&pm;`r round(sd(stellako.enviro$water_gauge_m,na.rm=T),2)` m), resulting in flooding of most of the lower river channels (Figure 1). The decline on April 27 is likely an effect of Francois Lake buffering the water gauge reading, which is at the top of the Stellako River near Glenannan. 

```{r, message=F, echo=F, warning=F}
enviro_plot(stellako.enviro, 0.2)
```
...
Catches are finally increasing at Stellako (Figure 2a). Smolt migration is anticipated to around early-mid May. Variability in hourly catch is still high, but so far suggests that smolts migrate later in the night at Stellako than at Nadleh (Figure 2b). 

```{r, warning=F, message=F, echo=F}
ggarrange(nightly_plot(stellako.catch, 300, 50), hourly_plot(stellako.catch), nrow=2)
```
...
Smolts encountered at the Stellako River site are getting larger, on average `r round(mean(stellako.bio.raw$length_mm,na.rm=T),1)`&pm;`r round(sd(stellako.bio.raw$length_mm, na.rm=T),1)` mm and `r round(mean(stellako.bio.raw$weight_g,na.rm=T),1)`&pm;`r round(sd(stellako.bio.raw$weight_g,na.rm=T),1)` g. 
...
*Table 1. Mark-release cohorts released on the Stellako River*
```{r, echo=F, warning=F, message=F}
stella.releases <- data.frame(Date=c("May 5"), cohort_size=c(114), mark_type=c("Upper caudal clip"))
...
*Table 2. Mark-release cohorts released on the Stellako River*
```{r, echo=F, warning=F, message=F}
nadleh.releases <- data.frame(Date=c("April 28", "May 1"), cohort_size=c(421, 1220), mark_type="Lower caudal clip")
...
Next update will be: `r Sys.Date()+7`
...
saveWorkbook(northern_wb, "Northern sockeye smolt programs 2021 - In-season data update #4.xlsx", overwrite = T)
```
